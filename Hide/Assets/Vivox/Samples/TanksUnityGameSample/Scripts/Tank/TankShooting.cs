...
public class TankShooting : NetworkBehaviour
{
    ...
    private bool ShootButtonDown => m_gameManager.ShootButton != null ? m_gameManager.ShootButton.GetComponent<ButtonState>().isDown : false;
    private bool ShootButtonPressed => m_gameManager.ShootButton !=null ? m_gameManager.ShootButton.GetComponent<ButtonState>().isPressed : false;
    private bool ShootButtonUp => m_gameManager.ShootButton != null ? m_gameManager.ShootButton.GetComponent<ButtonState>().isUp : false;
    ...
    private void Start()
    {
        ...
        m_isTouchInputEnabled = m_gameManager.ShootButton.IsActive();
    }
    ...
    private void Update()
    {
        if (!isLocalPlayer || !m_gameManager.IsGameInputEnabled)
            return;
        ...
        else if ((Input.GetButtonDown(m_FireButton) && !m_isTouchInputEnabled && noUiControlsInUse) || ShootButtonDown)
        {
        ...
        else if (((Input.GetButton(m_FireButton) && !m_isTouchInputEnabled && noUiControlsInUse) || ShootButtonPressed) && !m_Fired)
        {
        ...
        else if (((Input.GetButtonUp(m_FireButton) && !m_isTouchInputEnabled && noUiControlsInUse) || ShootButtonUp) && !m_Fired)
        {
            // ... launch the shell.
            Fire();
        }
    ...
    private void CmdFire(Vector3 rigidbodyVelocity, float launchForce, Vector3 forward, Vector3 position, Quaternion rotation)
    {
       ...
       shell.m_playerOriginTankSetup = m_tankSetup;
